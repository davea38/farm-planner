...
- [x] **Finding #31:** Typography and warmth audit complete — base font 18→20px per spec, all generic Tailwind colors replaced with farm-* palette tokens, text-red-500→text-destructive for validation errors, bg-white→bg-card for warm whites, Sparkline default color→#2e7d32, uppercase removed from section/chart labels (sentence case per spec). Result numbers consistently use text-4xl font-bold tabular-nums.

- [x] **Finding #32:** Empty states audit complete — all empty states now have icon + friendly headline + explanation + action button. ProfitabilityOverview navigates to Machines tab, ContractingIncomePlanner has "Could your machines earn more?" prompt, ReplacementPlanner timeline has two-line guidance. MachinesTab was already complete. Tabs (Compare, Replace, Contract, Worth It) are now navigable without a machine selected so users see empty-state prompts; machine banner restricted to cost-calculator/depreciation tabs only.
